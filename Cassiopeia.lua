--[[
	...
	| $$  | $$|__/      |_  $$_/                     /$$__  $$                             |__/                              |__/          
	| $$  | $$ /$$        | $$   /$$$$$$/$$$$       | $$  \__/  /$$$$$$   /$$$$$$$ /$$$$$$$ /$$  /$$$$$$   /$$$$$$   /$$$$$$  /$$  /$$$$$$ 
	| $$$$$$$$| $$        | $$  | $$_  $$_  $$      | $$       |____  $$ /$$_____//$$_____/| $$ /$$__  $$ /$$__  $$ /$$__  $$| $$ |____  $$
	| $$__  $$| $$        | $$  | $$ \ $$ \ $$      | $$        /$$$$$$$|  $$$$$$|  $$$$$$ | $$| $$  \ $$| $$  \ $$| $$$$$$$$| $$  /$$$$$$$
	| $$  | $$| $$        | $$  | $$ | $$ | $$      | $$    $$ /$$__  $$ \____  $$\____  $$| $$| $$  | $$| $$  | $$| $$_____/| $$ /$$__  $$
	| $$  | $$| $$       /$$$$$$| $$ | $$ | $$      |  $$$$$$/|  $$$$$$$ /$$$$$$$//$$$$$$$/| $$|  $$$$$$/| $$$$$$$/|  $$$$$$$| $$|  $$$$$$$
	|__/  |__/|__/      |______/|__/ |__/ |__/       \______/  \_______/|_______/|_______/ |__/ \______/ | $$____/  \_______/|__/ \_______/
																										 | $$                              
																										 | $$                              
																										 |__/                              
...
local UPDATE_PATH = "/Celtech/BOL/master/Cassiopeia.lua".."?rand="..math.random(1,10000)
local UPDATE_FILE_PATH = SCRIPT_PATH..GetCurrentEnv().FILE_NAME
local UPDATE_URL = "https://"..UPDATE_HOST..UPDATE_PATH
local ScriptName = "[Hi I'm Cassiopeia]"
...
if AUTOUPDATE then
	local ServerData = GetWebResult(UPDATE_HOST, "/Celtech/BOL/master/cassiopeia.version")
	if ServerData then
...
function OnLoad()
	if myHero.charName == "Cassiopeia" and tonumber(version) == ServerVersion then
...
function CastQ()
	if QREADY and ValidTarget(Target) then	
		local CastPosition, HitChance, Position = VP:GetCircularCastPosition(Target, 0.6, 12, 850, math.huge)
...
function CastE()
	if EREADY and ValidTarget(Target) then
		if TargetHaveBuff("cassiopeianoxiousblastpoison", Target) then
...
function CastW()
	if WREADY and ValidTarget(Target) then
		local CastPosition, HitChance, Position = VP:GetCircularCastPosition(Target, .6, 62, 850, math.huge)
		if CastPosition and HitChance >= Menu.misc.WHitChance and GetDistance(CastPosition) < 850 then
...
function CastR()
	if RREADY and ValidTarget(Target) then
		local CastPosition, HitChance, maxHit = VP:GetConeAOECastPosition(Target, .25, 80, 700, math.huge)
		if CastPosition and HitChance >= Menu.misc.RHitChance and GetDistance(CastPosition) < 700 then
			if maxHit >= Menu.misc.ult and isFacing(Target, myHero, 825) then
...
function LaneClearMode()
	if LCKey() then
		for _,minion in pairs(enemyMinions.objects) do
			...
			if EREADY and Menu.clear.useE and minion ~= nil and ValidTarget(minion, 600) and (((myHero.mana /  myHero.maxMana) * 100) > Menu.clear.EMana) and TargetHaveBuff("cassiopeianoxiousblastpoison", minion) then
				CastSpell(_E, minion)
			end
		end
	end
...
function GetQDmg(unit)
   ...
   local baseDmg = {25,38,51,65,78}
   local trueDmg = baseDmg[sLvl] + 0.15*myHero.ap
   local finalDmg =  trueDmg * 3
...
function drawDamage()
	for _, enemy in ipairs(GetEnemyHeroes()) do  
		if enemy.visible and not enemy.dead then
			if isFacing(myHero, enemy,500) then
				--DrawText3D(tostring("Facing"), enemy.x, enemy.y, enemy.z, 16, ARGB(255, 255, 255, 255), true)
			end
		end
	end
end
...
function InitMenu()
	Menu = scriptConfig("Cassiopeia", "CAS")
	...
	Menu.harass:addParam("useE", "Use E in Harass", 1, true)
	...
	Menu.misc:addParam("RHitChance", "R Hit Chance", SCRIPT_PARAM_SLICE, 2, 0, 2, decimalPlace)
	Menu.misc:addParam("ult", "Min Enemies To Ult", SCRIPT_PARAM_SLICE, 2, 1, 5, decimalPlace)
...
function OnDraw()
	if obwwillwork == true and tonumber(version) == ServerVersion then
		...
		if Menu.draw.r and RREADY then
			DrawCircle(myHero.x,myHero.y,myHero.z, 825, ARGB(255,255,255,255))
		end
